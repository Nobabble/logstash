...
When you start Logstash without arguments, it will read the `pipelines.yml` file and instantiate all pipelines specified in the file. On the other hand, when you use `-e`, `-f`, or '--path.config CONFIG_PATH', Logstash ignores the `pipelines.yml` file and logs a warning about it. (Note: Enabling "path.config:" in 'logstash.yml' also causes an ignore of 'pipelines.yml')
